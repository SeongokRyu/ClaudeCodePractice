...
`src/user-service.ts`는 사용자 CRUD 서비스입니다.
...
```
src/user-service.ts의 전체 코드를 읽고 각 함수의 역할을 설명해주세요.
```

### 1-2. 타입 분석

```
src/types.ts의 User 타입을 분석하고, 추가하면 좋을 필드를 제안해주세요.
```

### 1-3. 테스트 분석

```
src/user-service.test.ts의 테스트 커버리지를 분석하고,
...
```
src/user-service.ts에 이메일 중복 검사 기능을 추가해주세요.
createUser 함수에서 같은 이메일의 사용자가 이미 존재하면 에러를 throw하도록 수정해주세요.
테스트도 추가해주세요.
npm test로 검증해주세요.
```
...
```
src/user-service.ts에 다음 기능을 추가해주세요:
1. findByEmail(email: string) 함수
2. updateUser에서 이메일 변경 시 중복 검사
3. 각 기능에 대한 테스트
npm test로 검증해주세요.
```
...
```
user-service에 페이지네이션이 적용된 listUsers 함수를 추가해주세요.
