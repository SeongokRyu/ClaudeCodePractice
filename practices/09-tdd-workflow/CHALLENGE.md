...
`src/shopping-cart.test.ts`에는 쇼핑 카트 모듈의 테스트가 이미 작성되어 있습니다. 먼저 테스트가 실패하는 것을 확인하세요.
...
```
src/shopping-cart.test.ts의 테스트를 모두 통과하도록
src/shopping-cart.ts를 구현해줘.
...
테스트를 통과하는 구현을 작성해.
npm test로 확인하고, 실패하면 수정해서 모든 테스트가 통과할 때까지 반복해.
```
...
- [ ] Step 2: shopping-cart.ts를 구현하여 모든 테스트가 통과한다
