...
```
types.ts        ← 의존성 없음 (최하위)
database.ts     ← types.ts
user-repository.ts  ← database.ts, types.ts
order-repository.ts ← database.ts, types.ts
notification-service.ts ← types.ts
app.ts          ← 모든 모듈 (최상위)
```
...
```
리팩토링 계획의 1단계를 실행해줘.
database.ts를 콜백 패턴에서 async/await 패턴으로 변환해.

변환 후:
1. npm test를 실행해서 결과를 확인해
...
```
다음 파일(user-repository.ts)을 async/await로 리팩토링해줘.
이전 단계에서 변경한 database.ts의 새 인터페이스를 사용해.
변경 후 npm test를 실행해서 확인해.
```
