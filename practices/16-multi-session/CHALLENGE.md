...
```
src/rate-limiter-interface.ts의 RateLimiter 인터페이스를 구현하는
SlidingWindowRateLimiter 클래스를 작성해주세요.

요구사항:
- 슬라이딩 윈도우 알고리즘 사용
- src/sliding-window-limiter.ts에 구현
- 모든 테스트가 통과해야 합니다
```
...
- 파일: src/sliding-window-limiter.ts
- src/rate-limiter-interface.ts의 인터페이스를 구현
- src/rate-limiter.test.ts의 테스트를 통과해야 함
- 구현 선택의 이유를 주석으로 설명
```
...
- 파일: src/token-bucket-limiter.ts
- src/rate-limiter-interface.ts의 인터페이스를 구현
- src/rate-limiter.test.ts의 테스트를 통과해야 함
- 구현 선택의 이유를 주석으로 설명
```
...
```
src/sliding-window-limiter.ts와 src/token-bucket-limiter.ts를
다음 기준으로 비교 분석해주세요:
...
```
src/rate-limiter-interface.ts의 RateLimiter 인터페이스에 대한
실패하는 테스트를 3개 작성해주세요.

- 파일: src/tdd-limiter.test.ts
...
```
src/tdd-limiter.test.ts의 실패하는 테스트 3개를 통과하도록
TddRateLimiter를 구현해주세요.

- 파일: src/tdd-limiter.ts
- 최소한의 코드로 테스트를 통과시키세요
- 구현 후 npm test로 확인해주세요
```
...
```
src/tdd-limiter.test.ts에 추가 테스트를 3개 더 작성해주세요.
...
```
src/tdd-limiter.test.ts의 새로 추가된 테스트를 통과하도록
src/tdd-limiter.ts를 업데이트해주세요.
```
...
1. src/rate-limiter-service.ts — HTTP 서버
...
1. src/rate-limiter-service.test.ts
   - API 엔드포인트별 테스트
